...
\end{enumerate}			 
			...
			\begin{enumerate}
				...
				\iffalse						
				have a formula to evaluate $\lim_{\delta \to 0} \int_0^\delta \frac{1}{v(z)}dz$
				\fi
			\end{enumerate}
			We claim that for $x(0) = 1$ unique steady state solution occurs when $\alpha \geq 1$.  Thus gives us two cases to test:
			\begin{enumerate}
				\item Suppose $\alpha > 1$ Since we know that for $\alpha > 1$, $T_1 = \infty$, then we have evaluated on $(1-\delta,1)$.  Thus we only need to evaluate the following:
				$$
				\lim_{\delta \to 0} \int_{1}^{1+\delta} |u^{-\alpha}|du =  |\frac{-1}{k}(u(1+\delta)^k - u(1)^k)| = \infty	
				$$
				
				\item Suppose $\alpha = 1$. Since we know that for $\alpha = 1$, $T_1 = \infty$ then we have already evaluated on $(1-\delta, 1)$.  Thus we only need to evaluate the following:
				$$
				\lim_{\delta \to 0} \int_{1}^{1+\delta} |u^{-1}|du =  \lim_{\delta \to 0} |-ln|u(1)| + ln|u(\delta)|| = \infty.
				$$
			\end{enumerate}
			...
			\end{equation*}
			Since $v'$ is unbounded then the $\alpha = 1$ case is not Lipschitz.
		\end{enumerate}
		\item[2.10]
		\begin{enumerate}
