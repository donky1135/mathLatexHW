...
def confusion_matrix( model, x, y ):
    ...
    logits = model( x )
    # Note the model outputs logits rather than probabilities
    # But the max probaiblity corresponds to the max logit
...
for epochs in range(10):
    ...
    for batch in range( TRAIN_N // BATCH_SIZE ):
        ...
        correct_class_logits = torch.gather( logits, dim = 1, index = torch.reshape( y_batch, (-1,1) ) ) 
        loss = torch.mean( -1 * correct_class_logits )
        # No longer need to take logs or add the smoothing factor from before
