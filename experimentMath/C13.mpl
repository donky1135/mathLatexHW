...
#beginning of leacture 13

#note that the rate of a code is the size of the subspace, k, divided by the number of inputs (ambient dimension), k/n

#CCshop(n,q,x,K): shopping for codes with codwrods of length n ver GF(q) Fqn(q,n) 
#with at most K code words if M is a basis corresponding to g  (q^nops(M) <= K)
CCshop := proc(n,q,x,K) local C,S,M,g:
    S := AllFactors(n,q,x):
    C := {}:
    for g  in S do 
        M := CtoL(n,x,g):
        if q^nops(M) <= K then 
            C := C union {[g,evalf(nops(M)/n),MinW(q,M)]}
        fi:
    od:
    RETURN(C):
end:

#V(a,r): the r by r Vandermonde matrix whose i,j-th entry is a[j]^(i-1)

V := proc(a,r) local i:
[seq([seq(a[i]^j,i=1..r)], j=0..(r-1))]:
end:
