...
#1
CCshopF := proc(n,q,x,K,R,W) local C,S,M,g,w:
S:=AllFactors(n,q,x):
C:={}:

for g in S do
  M:=CtoL(n,x,g):
  if q^nops(M)<=K and evalf(nops(M)/n) >= evalf(R) and MinW(q,M) >= W  then
   w:=MinW(q,M):
   C:=C union {[g,evalf(nops(M)/n),w]}:
  fi:
od:
...
#Note for all requested primes q=2,3,5, CCshopF(30,q,x,10000,1/2,5) just returns the empty set

#2
CtoDual := proc(n,q,g,x) local h,r,i,j:
    h := modp(quo((x^n - 1),g,x),q):
    r := degree(h,x):
    seq([0$(i-1),seq(coeff(h,x,r-j),j=0..r),0$(n-r -i)], i=1..(n-r)):
end:

#proof that det(V(a,r)) = dV(a,r)
#Note for a 1 by 1 vandermonde matrix we have that 
# V(a,1) = 1, which since 1 !< 1 implies that the product \prod_{i < j} (a_j - a_i) is 1
# Assume that the induction hypothesis holds for 1 to r-1.  
# Then given the Vandermonde matrix V(a,r) = [[1,..,1],[a_1,..,a_r],...,[a_1^{r-1},..,a_r^{r-1}]],
# we can do the row operations R_{i+1} - a_1 * R_i -> R_{i+1} simultaneously to 
# get the matrix with equivalent determinate, 
# A = [[1,..,1],[0, a_2 - a_1,..,a_r - a_1],...,[0,a_2^{r-2}(a_2 - a_1),..,a_r^{r-2}(a_r - a_1)]].
# Thus det(V(a,r)) = det(A) = det([[a_2 - a_1,..,a_r - a_1],...,[a_2^{r-2}(a_2 - a_1),..,a_r^{r-2}(a_r - a_1)]]).
# Furthermore, since one can factor out constant factors multiplied by columns in determinates, 
# det(A) = (a_2 - a_1)(a_3 - a_1)...(a_r - a_1)det([[1,...,1],...,[a_2^{r-2},..,a_r^{r-2}]])
# since [[1,...,1],...,[a_2^{r-2},..,a_r^{r-2}]] is a vandermonde matrix, one can apply the induction 
# hypothesis to yield det([[1,...,1],...,[a_2^{r-2},..,a_r^{r-2}]]) = \prod_{i < j, i > 1} (a_j - a_i).
# Since the outside factor of (a_2 - a_1)(a_3 - a_1)...(a_r - a_1) is exactly \prod_{1 < j}(a_j - a_1), 
# then det(V(a,r)) = \prod_{i < j} (a_j - a_i).  QED.
