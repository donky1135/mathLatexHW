...
#C14.txt, March 4, 2024
Help:=proc(): print(`Encode113(x) , RV113(), Sy113(x) ,Decode113(y) `):end:

#old code
#Feb. 29, 2024 C13.txt
Help13:=proc(): print(` CCshop(n,q,x,K), V(a,r),  `):end:
...
if eq=0 then
 i:=S[2]/S[1] mod 11:
 a:=S[1]:
  RETURN([op(1..6,y-[0$(i-1),a,0$(nops(y)-i)])] mod 11):
fi:
...
a:=S[1]-b mod 11:
[op(1..6,y-[0$(i-1),a,0$(j-i-1),b,0$(nops(y)-j)] mod 11)]:
end:

#beginning of homework 14

Encode113G := proc(x,q) local a,b,c,d,n,eq1,eq2,eq3,eq4,i,S:
    n := nops(x) + 4:
    if (n > q-1) then RETURN(FAIL): fi:
    eq1 := 0 = add(x[i], i=1..(n-4)) + a + b + c + d:
    eq2 := 0 = add(i * x[i], i=1..(n-4)) + (n-3)*a + (n-2)*b + (n-1)*c + (n)*d:
    eq3 := 0 = add(i^2 * x[i], i=1..(n-4)) + (n-3)^2*a + (n-2)^2*b + (n-1)^2*c + n^2*d:
    eq4 := 0 = add(i^3 * x[i], i=1..(n-4)) + (n-3)^3*a + (n-2)^3*b + (n-1)^3*c + n^3*d:
