...
#beginning of homework 10:

#helper function which makes a list of lists in lexigraphical order of length n
lexigraph := proc(q,n) local i,l,L:
    option remember:
    if n = 1 then 
        RETURN([seq([i],i=0..q-1)]):
    fi:
    L := lexigraph(q,(n-1)):
    seq(seq([i,op(l)], l in L),i=0..q-1):
end:

Hampcm := proc(r,q) local H, i, l, L, L1:
    H := [[0$(r-1),1]]:
    for i from 1 to (r-1) do
        L := lexigraph(q,i):
        L1 := seq([0$(r-i-1),1,op(l)], l in L):
        H := [op(H),L1]:
    od:
    H := SFde(q,TRA(H)):
    n := nops(H[1]):
    TH := TRA(H):
    TRA([op(r+1..n,TH(H)),op(1..r,TH)]):
end:

HampcmOG := proc(r,q) local H, i, l, L, L1:
    H := [[0$(r-1),1]]:
    for i from 1 to (r-1) do
        L := lexigraph(q,i):
        L1 := seq([0$(r-i-1),1,op(l)], l in L):
...
DecodeHamming := proc(q,r,v) local H,y,j,n:
    H := HampcmOG(r,q):
    y := Syn(q,H,v): 
    b := y[1] mod q:
    if b <> 1 then
    ...
    # print(y);
    # print(b);
    # print(b*[0$(j-1),1,0$(n-j-1)] mod q);

    # print(n);
    (v-b*[0$(j-1),1,0$(n-j-1)] mod q):
end: